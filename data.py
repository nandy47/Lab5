...
This also includes columns relevant to the Stage A leakage check: the real OpenML
Titanic carries boat, body, and home.dest, and this file's rename map deliberately
leaves them untouched, so they pass through into the DataFrame like any other column -
nothing marks them as special. Look at what each one means before deciding whether to
use it. The synthetic fallback reproduces all three so this check works the same way
regardless of which data path you get.
...
def _synthetic_titanic(n=891, seed=RANDOM_STATE):
    ...
    Also reproduces boat/body/home.dest from the real dataset - see the module
    docstring.
    """
    ...
    # --- boat / body / home.dest, mirroring the real OpenML dataset ---
    df["boat"] = pd.Series(rng.integers(1, 16, n).astype(str), dtype="object")
    has_boat = rng.random(n) < np.where(survived == 1, 0.95, 0.02)
    df.loc[~has_boat, "boat"] = np.nan

    df["body"] = rng.integers(1, 330, n).astype(float)
    has_body = rng.random(n) < np.where(survived == 0, 0.17, 0.0)
    df.loc[~has_body, "body"] = np.nan

    dest_cities = ["New York, NY", "London, England", "Boston, MA", "Philadelphia, PA",
                   "Chicago, IL", "Cork, Ireland", "Paris, France", "Toronto, Ontario"]
    df["home.dest"] = pd.Series(rng.choice(dest_cities, n), dtype="object")
    has_dest = rng.random(n) < np.where(pclass == 1, 0.85, np.where(pclass == 2, 0.55, 0.25))
    df.loc[~has_dest, "home.dest"] = np.nan
